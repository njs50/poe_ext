...
function getSocketLinkage(itemDiv) {
  ...
  if (aSockets.length === 0) {
    return 0;
  }

  if (aSockets.length === 1) {
    return 2;
  }
...
function calcAvRange(range) {
  ...
  if (range.indexOf('-') > 0) {
    ...
    aTemp2 = range.split(' to ');
  }

  return ( parseInt(aTemp2[0],10) + parseInt(aTemp2[1],10) ) / 2;
}
...
function getPropertyOrModsInt(item,prop) {
  ...
  if (item.implicitMods.hasOwnProperty(prop)) {
    amt += parseInt(item.implicitMods[prop], 10);
  }
  if (item.explicitMods.hasOwnProperty(prop)) {
    amt += parseInt(item.explicitMods[prop], 10);
  }
...
function combineMods(explicitMods, implicitMods, craftedMods){
  ...
  $.each([implicitMods, craftedMods], function(idx, implicitMods) {
    ...
    for (var key in implicitMods) {
      if (oCombined.hasOwnProperty(key)) {
        ...
        if (a.indexOf('-') > 0){
          ...
          a = a.split('-');
          b = b.split('-');
          oCombined[key] = (parseInt(a[0], 10) + parseInt(b[0], 10)) + '-' + (parseInt(a[1], 10) + parseInt(b[1], 10));
        } else if (a.indexOf('%') > 0) {
          // percents
          ...
          oCombined[key] = parseInt(a, 10) + parseInt(b, 10);
        }
...
function itemByName(items, name) {
  return $(items.filter(function(i){
    return $(':contains(' + name + ')', $(i.html)).length > 0; })[0].html);
}
